           ...
           @auth
               @if ($post->user_id == auth()->user()->id)
                <form action="{{route('post.destroy',$post)}}" method="POST">
                    ...
                    <button type="submit" class="bg-red-500 hover:bg-red-600 p-2 rounded text-white font-bold mt-4 cursor-pointer">Eliminar publicacion</button>
                </form>
               @endif
           @endauth
                    ...
                    <form action="{{route('comment.store',['user'=>$user,'post'=>$post])}}" method="POST">
                        ...
                        <div class="mb-5">
                            ...
                            <textarea
                                ...
                                class="border  w-full rounded-lg block pl-10 p-2.5 @error('comment') border-red-500 @enderror"
                            ></textarea>
                        ...
                        <button type="submit" class="bg-sky-600 hover:bg-sky-700 transition-colors cursor-pointer uppercase font-bold w-full p-3 text-white rounded-lg">Comentar</button>
                    </form>
                @endauth
                    ...
                    @if ($post->comments->count())
                        @foreach ($post->comments as $comment )
                            <div class="p-5 border-gray-300 border-b">
                                <a href="{{route('post.index',$comment->user->username)}}" class="font-bold">{{$comment->user->username}}</a>
                                <p>{{$comment->comment}}</p>
